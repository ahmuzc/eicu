drop table if exists dopamine_info_eicu;
create table dopamine_info_eicu as
...
, dopamine_in_part_2 as (
  ...
  , nip.drugname 
  , nip.unit_flag
  , LAST_VALUE(nip.unit_flag) OVER (partition by nip.patientunitstayid order by infusionoffset ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as unit_flag_locf
  , first_VALUE(nip.unit_flag) OVER (partition by nip.patientunitstayid order by infusionoffset ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as unit_flag_focb
