drop table if exists norepinephrine_info_eicu;
create table norepinephrine_info_eicu as
...
, norepinephrine_in_part_2 as (
  ...
  , nip.drugname 
  , nip.unit_flag
  , LAST_VALUE(nip.unit_flag) OVER (partition by nip.patientunitstayid order by infusionoffset  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as unit_flag_locf
  , first_VALUE(nip.unit_flag) OVER (partition by nip.patientunitstayid order by infusionoffset RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as unit_flag_focb
...
, norepinephrine_1 as (
  ...
  left join weight_icustay_detail_modify_eicu wi 
